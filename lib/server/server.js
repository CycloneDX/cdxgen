...
export function isAllowedWinPath(p) {
  // This is tricky. We have to rely on the vulnerable normalize function
  const { rootDrive } = path.win32.parse(path.win32.normalize(p));
  return rootDrive ? /^[A-Za-z]:\\$/.test(rootDrive) : true;
}
