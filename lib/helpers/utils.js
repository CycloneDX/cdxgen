...
export function getAllFiles(dirPath, pattern, options = {}) {
  ...
  return getAllFilesWithIgnore(dirPath, pattern, includeDot, ignoreList);
}
...
export function mapConanPkgV2RefToPurlStringAndNameAndVersion(conanPkgRef) {
...
export function parseConanLockData(conanLockData) {
  ...
  if ((!lockFile || !lockFile.graph_lock || !lockFile.graph_lock.nodes) && !lockFile.requires) {
    return pkgList;
  }
  if (lockFile.graph_lock && lockFile.graph_lock.nodes) {
    const depend = lockFile.graph_lock.nodes;
    for (const nk of Object.keys(depend)) {
      if (depend[nk].ref) {
    ...
    const depend = lockFile.requires;
    for (const nk of Object.keys(depend)) {
      const [purl, name, version] = mapConanPkgV2RefToPurlStringAndNameAndVersion(
        depend[nk]
      );
      if (purl !== null) {
        pkgList.push({
