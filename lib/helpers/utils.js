...
export function parseFlakeNix(flakeNixFile) {
  ...
  try {
    ...
    let inputsContent = "";
    const inputsStart = flakeContent.search(/inputs\s*=\s*\{/);
    ...
    if (inputsStart !== -1) {
      ...
      for (let i = inputsStart; i < flakeContent.length; i++) {
        const char = flakeContent[i];
        if (char === "{") {
          if (!started) {
            started = true;
            startPos = i + 1;
          ...
          if (braceCount === 0 && started) {
            inputsContent = flakeContent.slice(startPos, i);
            break;
          }
        }
      }
    ...
    if (inputsContent) {
      ...
      const inputPatterns = [
        ...
        /([a-zA-Z0-9_-]+)\.url\s*=\s*"([^"]+)"/g,
        // Pattern: nixpkgs = { url = "github:NixOS/nixpkgs"; };
        /([a-zA-Z0-9_-]+)\s*=\s*\{\s*url\s*=\s*"([^"]+)"[^}]*\}/gs,
      ];

      const addedPackages = new Set(); // Track added packages to avoid duplicates
