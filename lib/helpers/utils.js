...
export function parseFlakeNix(flakeNixFile) {
  ...
  try {
    ...
    if (inputsMatch) {
      ...
      const inputPatterns = [
        // Pattern: nixpkgs.url = "github:NixOS/nixpkgs/release-23.11";
        /(\w+)\.url\s*=\s*"([^"]+)"/g,
        // Pattern: nixpkgs = { url = "github:NixOS/nixpkgs"; };
        /(\w+)\s*=\s*\{\s*url\s*=\s*"([^"]+)"[^}]*\}/g,
      ];

      for (const pattern of inputPatterns) {
        let match;
        while ((match = pattern.exec(inputsContent)) !== null) {
          ...
          if (name && url) {
            ...
            // Generate PURL based on the URL type
            pkg.purl = generateNixPurl(name, url, "latest");
  ...
  try {
    ...
    if (lockData.nodes) {
      for (const [nodeName, nodeData] of Object.entries(lockData.nodes)) {
        // Skip the root node
        if (nodeName === "root" || !nodeData.locked) {
          continue;
        }
        ...
        // Determine version from lock data
        let version = "latest";
        if (locked.rev) {
          version = locked.rev.substring(0, 7); // Short commit hash
        } else if (locked.ref) {
          version = locked.ref;
        }
        ...
        // Generate PURL
        const inputUrl = generateInputUrl(locked, original);
        pkg.purl = generateNixPurl(nodeName, inputUrl, version);
        pkg["bom-ref"] = pkg.purl;
      ...
      if (lockData.root && lockData.root.inputs) {
        const rootInputs = Object.keys(lockData.root.inputs);
        if (rootInputs.length > 0) {
          dependencies.push({
            ref: "pkg:nix/flake@latest", // Root flake reference
            dependsOn: rootInputs.map(input => 
 ...
 *
 * @param {String} name Package name
 * @param {String} url Input URL or generated URL
 * @param {String} version Package version
 * @returns {String} PURL string
 */
function generateNixPurl(name, url, version) {
...
/**
 * Generate input URL from lock data
 *
 * @param {Object} locked Locked dependency data
 * @param {Object} original Original dependency data
 * @returns {String} Reconstructed input URL
 */
function generateInputUrl(locked, original) {
  if (locked.type === "github") {
    return `github:${locked.owner}/${locked.repo}`;
  } else if (locked.type === "gitlab") {
    return `gitlab:${locked.owner}/${locked.repo}`;
  } else if (locked.type === "git") {
    return locked.url || original.url || "unknown";
  } else if (locked.type === "path") {
    return `path:${locked.path || original.path}`;
  } else if (original.url) {
    return original.url;
  }
  
  return `${locked.type || 'unknown'}:${locked.owner || ''}/${locked.repo || locked.path || 'unknown'}`;
}
...
export function executeAtom(src, args, extra_env = {}) {
  ...
  if (result.stderr) {
    if (
      ...
      console.warn(
        "WARN: Unable to locate astgen command. Install atom globally using sudo npm install -g @appthreat/atom-parsetools@1.0.4 to resolve this issue.",
      );
    } else if (
      result.stderr?.includes(
