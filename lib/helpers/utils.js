...
import { TRACE_MODE, thoughtLog, traceLog } from "./logger.js";
...
function isAllowedCommand(command) {
  ...
  // Normalize command and allowlist 
  const normalizedCommand = basename(command.trim());
  const allow_commands = (process.env.CDXGEN_ALLOWED_COMMANDS || "")
    .split(",")
    .map(cmd => basename(cmd.trim()));
  return allow_commands.includes(normalizedCommand);
}
...
export function safeSpawnSync(command, args, options) {
  if (
    ...
    // Print language specific commands suggestion
    const normalizedCommand = basename(command.trim());
    if (!warnedCommands.has(normalizedCommand)) {
      warnedCommands.add(normalizedCommand);
      console.log(
          `\x1b[1;33mSECURE MODE: The command '${normalizedCommand}' is not allowed.\x1b[0m`
        );
      const allowed = process.env.CDXGEN_ALLOWED_COMMANDS
        ? `${process.env.CDXGEN_ALLOWED_COMMANDS},${normalizedCommand}`
        : normalizedCommand;
      console.log(
        `To allow it, set: CDXGEN_ALLOWED_COMMANDS="${allowed}"`
      );
    }
    return {
      status: 1,
