...
export function buildGradleCommandArguments(
  ...
  const maxCliArgsLength = isWin ? 8191 - gradleCommandLength - mainGradleArguments.join(" ").length - 2 : -1;
  const splitArgs = [];
  ...
  for (const gradleSubCommand of gradleSubCommands) {
    const subCommandLength = [gradleSubCommand, ...gradleSubCommandArguments].join(" ").length + 1;
