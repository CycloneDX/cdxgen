...
export function executeAtom(src, args, extra_env = {}) {
  ...
  if (result.stderr) {
    ...
    } else if (result.stderr?.includes("Could not parse command line options")) {
      console.warn("Invalid command-line options passed to atom. Please file a bug in the cdxgen repository.");
    }
  }
  if (result.stdout) {
