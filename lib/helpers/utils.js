...
export function isPackageManagerAllowed(name, conflictingManagers, options) {
  ...
  if (res) {
    thoughtLog(
      `Let's make use of the package manager ${name}, which is allowed.`,
    );
  }
  return res;
}
      ...
      if (result.status !== 0 || result.error) {
        ...
        if (!versionRelatedError) {
          if (DEBUG_MODE) {
            ...
            if (process.env?.CDXGEN_IN_CONTAINER !== "true") {
              thoughtLog(
                "Installation of build dependencies failed. If in doubt, use the cdxgen container images for python.",
              );
            } else {
              thoughtLog(
