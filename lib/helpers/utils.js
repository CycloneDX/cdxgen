  ...
  if (existsSync(pkgJsonFile)) {
    try {
      ...
      if (DEBUG_MODE && !name && !pkgJsonFile.includes("node_modules")) {
        console.log(
          `${pkgJsonFile} doesn't contain the package name. Consider using the 'npm init' command to create a valid package.json file for this project. Ignore this warning in case of internal workspaces.`,
        );
        return pkgList;
      }
...
export async function parsePyLockData(lockData, lockFile, pyProjectFile) {
  ...
  if (pyProjectFile && existsSync(pyProjectFile)) {
    if (DEBUG_MODE) {
      console.log(
