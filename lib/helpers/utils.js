...
export async function collectMvnDependencies(
  ...
  if (basePath && basePath !== MAVEN_CACHE_DIR) {
    const argsToSanitize = ["-Dserver.username=", "-Dserver.password="];
    const sanitizedCopyArgs = copyArgs.map((arg) => {
      if (argsToSanitize.some((prefix) => arg.startsWith(prefix))) {
        const [key] = arg.split("=");
        return `${key}=***`;
      }
      return arg;
    ...
    console.log(
      `Executing '${mavenCmd} ${sanitizedCopyArgs.join(" ")}' in ${basePath}`,
    );
