...
export async function parsePodfileLock(podfileLock, projectPath) {
  ...
  if (["true", "1"].includes(process.env.COCOA_MERGE_SUBSPECS)) {
 ...
 *
 * @param {string} name The simple name of the pod
 * @param {object} metadata Object with all other parsed data for the pod
...
export async function buildObjectForCocoaPod(
  ...
  let component;
  if (
    dependency.properties?.find(({ name }) => name === "projectDir") &&
    ["true", "1"].includes(process.env.COCOA_RESOLVE_FROM_NODE)
  ) {
    ...
    if (
      tmpDir &&
      !exclusionDirs.some((dir) => `${tmpDir.replaceAll("\\", "/")}/`.includes(`/${dir.replaceAll("\\", "/")}/`)) &&
  ...
  if (!component) {
    ...
    if (options && ["true", "1"].includes(process.env.COCOA_FULL_SCAN)) {
...
function fullScanCocoaPod(dependency, component, options) {
  let result;
  if (component.properties?.find(({ name }) => name === "projectDir")) {
    if (component.properties.find(({ name }) => name === "podspec")) {
