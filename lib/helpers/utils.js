...
async function getVersionNumPnpm(depPkg) {
  let version = depPkg;
  ...
  if (version.startsWith("link:")) {
    version = version.replace("link:", "");
    if (
      existsSync(join(version.replaceAll("../", "packages/"), "package.json"))
    ) {
    ...
    if (ppurl !== "" && yamlObj?.importers) {
      ...
      for (const importedComponentName of Object.keys(yamlObj?.importers)) {
        ...
        if (existsSync(join(importedComponentName, "package.json"))) {
          ...
          const version =
            importedComponentObj[0].version ||
            parentComponent.version ||
            "latest";
        ...
        for (const cdk of Object.keys(componentDeps)) {
          ...
          const version = await getVersionNumPnpm(versionObj);
          const dpurl = new PackageURL(
            "npm",
            "",
            cdk,
