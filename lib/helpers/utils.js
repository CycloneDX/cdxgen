...
export function parsePyProjectTomlFile(tomlFile) {
  ...
  if (tomlData?.project?.dependencies) {
    for (const adep of tomlData.project.dependencies) {
      ...
      directDepsKeys[adep.split(" ")[0]] = true;
    }
  }
  if (tomlData?.project?.["dependency-groups"]?.length) {
    for (const agroup of Object.keys(tomlData.project["dependency-groups"])) {
      groupDepsKeys[agroup] = tomlData.project["dependency-groups"][agroup].map(
        (p) => p.split(" ")[0],
      );
    }
  }
    ...
    if (apkg.dependencies) {
      if (Array.isArray(apkg.dependencies)) {
        ...
        for (const apkgDep of apkg.dependencies) {
          ...
          const nameStr = apkgDep.name || apkgDep.split(" ")[0].split(">=")[0];
