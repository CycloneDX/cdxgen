...
export function safeSpawnSync(command, args, options) {
  if (isSecureMode && process.permission) {
    if (!process.permission.has("ChildProcess")) {
      if (DEBUG_MODE) {
  ...
  if (options?.shell === true) {
    if (args?.length) {
      command = `${command} ${args.map((a) => a.replaceAll(" ", "\\ ")).join(" ")}`;
      args = undefined;
    }
    if (DEBUG_MODE && command.includes("\\ ")) {
      console.log("Escaped command to execute:", command);
    }
  }
