...
function _quoteArg(arg) {
  ...
  if (isWin) {
    // wrap in double-quotes, escape embedded " and \
...
export function safeSpawnSync(command, args, options) {
  ...
  const quotedArgs = rawArgs.map(_quoteArg);
  // DEP0190 fix: if shell:true we need a single command string
  if (options?.shell === true) {
    const cmd = [command, ...quotedArgs].join(" ");
    return spawnSync(cmd, { ...options, shell: true });
  }
  return spawnSync(command, quotedArgs, options);
}
