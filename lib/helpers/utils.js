    ...
    beforeRequest: [
      (options) => {
        if (!isAllowedHost(options.url.hostname)) {
          if (TRACE_MODE) {
            console.log(
              `Access to remote host '${options.url.hostname}' is not allowed.`,
            );
          }
          return new AbortController().abort();
        }
        // Do not allow http protocol in secure mode
        if (isSecureMode && options.url.protocol === "http:") {
          console.log(
            `Access to remote host '${options.url.hostname}' is not allowed over http.`,
          );
          return new AbortController().abort();
        }
        traceLog("http", {
