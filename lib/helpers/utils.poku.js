...
it("parse cargo toml", async () => {
  ...
  assert.deepStrictEqual(dep_list, [
    ...
    },
    {
      ...
      purl: "pkg:cargo/libfuzzer-sys@git+https:%2F%2Fgithub.com%2Frust-fuzz%2Flibfuzzer-sys.git",
      "bom-ref":
...
it("parsePnpmLock", async () => {
  ...
  assert.deepStrictEqual(parsedList.pkgList.length, 353);
  assert.deepStrictEqual(parsedList.dependenciesList.length, 353);
