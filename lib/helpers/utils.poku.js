...
it("findPnpmPackagePath", () => {
  ...
  if (packagePath) {
    assert.deepStrictEqual(packagePath).toMatch(/node_modules.*chalk/);
  ...
  if (scopedPackagePath) {
    assert
      .deepStrictEqual(scopedPackagePath)
      .toMatch(/node_modules.*@babel.*core/);
  }
});
...
it("pnpmMetadata enhancement", async () => {
  ...
  if (chalkPkg) {
    ...
    if (localPath) {
      assert.deepStrictEqual(localPath.value).toMatch(/node_modules.*chalk/);
      assert.deepStrictEqual(chalkPkg.description);
      assert.deepStrictEqual(chalkPkg.license);
    }
  }
...
const testCases = [
  ...
  ["http://example.com/a%" + "a%".repeat(500), false], // Many %a patterns
  ...
  ["http://example.com/path%" + "20".repeat(300) + "%2", false], // Valid %20s, ends with incomplete %
  ["http://example.com/path%" + "20".repeat(300) + "a", false], // Valid %20s, ends with non-hex
]
