...
test("parse ts proj", () => {
  expect(parseTsProjData(null)).toEqual([]);
  let retMap = parseTsProjData(
    readFileSync("./test/sample.tsproj", { encoding: "utf-8" }),
  );
  expect(retMap?.parentComponent["bom-ref"]).toBeUndefined();
  expect(retMap.pkgList.length).toEqual(3);
  expect(retMap.pkgList[0]).toEqual({
    "bom-ref": "pkg:nuget/Microsoft.TypeScript.MSBuild@3.9.7",
    group: "",
    name: "Microsoft.TypeScript.MSBuild",
    version: "3.9.7",
    purl: "pkg:nuget/Microsoft.TypeScript.MSBuild@3.9.7",
  });
  expect(retMap?.parentComponent.properties).toEqual([
    { name: "cdx:dotnet:target_framework", value: "netcoreapp3.1" },
  ]);
...
test("parse plc proj", () => {
  expect(parsePlcProjData(null)).toEqual([]);
  let retMap = parsePlcProjData(
    readFileSync("./test/sample.plcproj", { encoding: "utf-8" }),
  );
  expect(retMap?.parentComponent["bom-ref"]).toBeUndefined();
  expect(retMap.pkgList.length).toEqual(4);
  expect(retMap.pkgList[0]).toEqual({
    "bom-ref": "pkg:nuget/Microsoft.PLC.MSBuild@1.0.0",
    group: "",
    name: "Microsoft.PLC.MSBuild",
    version: "1.0.0",
    purl: "pkg:nuget/Microsoft.PLC.MSBuild@1.0.0",
  });
  expect(retMap?.parentComponent.properties).toEqual([
    { name: "cdx:dotnet:target_framework", value: "netcoreapp3.1" },
  ]);
