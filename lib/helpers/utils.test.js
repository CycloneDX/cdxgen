...
test("parsePnpmLock", async () => {
  ...
  expect(parsedList.pkgList.length).toEqual(594);
  expect(parsedList.dependenciesList.length).toEqual(594);
  expect(parsedList.pkgList[0]).toEqual({
    group: "@ampproject",
    name: "remapping",
    version: "2.3.0",
...
test("parse pyproject.toml with poetryv2 requirement", () => {
  ...
  expect(retMap.parentComponent).toEqual({
    ...
    license: "MIT",
    authors: [{ name: "Team AppThreat", email: "cloud@appthreat.com" }],
    ...
    type: "application",
    "bom-ref": "pkg:pypi/blint@2.4.2",
    purl: "pkg:pypi/blint@2.4.2",
    evidence: {
      identity: {
        field: "purl",
        confidence: 1,
