...
test("csproj version extraction tests", () => {
  const testCases = [
  ...
  testCases.forEach((testCase, _index) => {
    ...
    expect(version).toEqual(testCase[1]);
  });
});

test("csproj package info extraction tests", () => {
  ...
  packageInfoTestCases.forEach((testCase, _index) => {
    const result = extractPackageInfoFromHintPath(testCase[0]);
    expect(result.name).toEqual(testCase[1]);
    expect(result.version).toEqual(testCase[2]);
  });
});

test("version validation tests", () => {
  const validVersions = [
    // Standard versions
    "1.0.0",
    "2.6.2",
  ...
  // Test valid versions
  validVersions.forEach((version, _index) => {
    expect(isValidVersion(version)).toBe(true);
  });

  // Test invalid versions
  invalidVersions.forEach((version, _index) => {
    expect(isValidVersion(version)).toBe(false);
  });
});

// Additional specific tests
test("should handle string representations of valid versions", () => {
  expect(isValidVersion("1.0.0")).toBe(true);
  expect(isValidVersion(String("2.6.2"))).toBe(true);
});
...
test("should handle boundary cases", () => {
  expect(isValidVersion("0.0.0.0")).toBe(true); // Four zeros
  expect(isValidVersion("999999.999999.999999")).toBe(true); // Large numbers
  expect(isValidVersion("1.0.0-")).toBe(false); // Empty pre-release
  expect(isValidVersion("1.0.0-a")).toBe(true); // Single char pre-release
});
