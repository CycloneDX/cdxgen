...
export function textualMetadata(bomJson) {
  ...
  if (bomPkgTypes.length && bomPkgNamespaces.length) {
    if (bomPkgTypes.length === 1) {
      text = `${text} The package type in this ${bomType} is ${joinArray(bomPkgTypes)} with ${bomPkgNamespaces.length} namespaces described under components.`;
    } else {
  ...
  if (bomType === "OBOM") {
    if (vsixCount > 0) {
      text = `${text} The OS has ${vsixCount} Visual Studio Code Extensions installed and hence resembles a remote development environment.`;
    }
    if (swidCount > 0) {
      text = `${text} There are ${swidCount} applications installed locally.`;
    }
  }
  return text;
}
