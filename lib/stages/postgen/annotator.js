...
export function textualMetadata(bomJson) {
  ...
  let text = "";
  const cdxTypeDesc = "Software Bill-of-Materials (SBOM)";
  ...
  if (metadata?.lifecycles) {
    ...
    if (lifecycles && Array.isArray(lifecycles)) {
      if (lifecycles.length === 1) {
        text = `${text} The data was captured during the ${lifecycles[0].phase} lifecycle phase.`;
      } else {
  ...
  if (metadata?.component) {
    ...
    if (
      ...
      text = `${text} The document describes ${toArticle(metadata.component.type)} ${metadata.component.type.replaceAll("-", " ")} named '${metadata.component.name}' with version '${parentVersion}'.`;
    } else {
      text = `${text} The document describes ${toArticle(metadata.component.type)} ${metadata.component.type.replaceAll("-", " ")} named '${metadata.component.name}'.`;
    }
    if (
      metadata?.component.components &&
      Array.isArray(metadata.component?.components)
    ) {
      text = `${text} The ${metadata.component.type.replaceAll("-", " ")} also has ${metadata.component.components.length} child modules/components.`;
    }
  }
  ...
  if (bomPkgTypes.length && bomPkgNamespaces.length) {
    if (bomPkgTypes.length === 1) {
      text = `${text} The package type in this SBOM is ${joinArray(bomPkgTypes)} with ${bomPkgNamespaces.length} namespaces described under components.`;
    } else {
...
export function extractTags(component) {
  ...
  for (const adescTag of componentTags.description) {
    if (
      desc &&
      (desc.includes(` ${adescTag} `) || desc.includes(` ${adescTag}.`))
    ) {
      tags.add(adescTag);
    }
  }
  // Identify tags from properties
