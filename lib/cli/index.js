    ...
    for (const f of filesToRestore) {
      ...
      if (DEBUG_MODE && (result.status !== 0 || result.error)) {
        if (
          ...
          console.log(
            `This project needs to be restored using msbuild. Example: 'msbuild -t:restore'. cdxgen is attempting to use ${buildCmd}, which might result in an incomplete SBOM!`,
          );
          if (process.env?.CDXGEN_IN_CONTAINER !== "true") {
            console.log(
        ...
        } else if (result?.stderr?.includes("is not found on source")) {
          console.log(
            `The project ${f} refers to private packages that are not available on nuget.org!`,
          );
          console.log(
            "Tip: Authenticate with any private registries such as Azure Artifacts feed before running cdxgen.",
          );
