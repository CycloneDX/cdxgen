  ...
  if (requirementsMode || setupPyMode || pyProjectMode || options.deep) {
    ...
    if (options.installDeps) {
      ...
      if (pyProjectMode && !poetryMode) {
        ...
        pkgMap = await getPipFrozenTree(path, setupPy, tempDir, parentComponent);
        ...
        for (const f of reqFiles) {
          ...
          let frozen = false;
          if (options.installDeps) {
          const loopPkgMap = await getPipFrozenTree(
            basePath,
            f,
            tempDir,
            parentComponent,
          ...
          if (loopPkgMap.pkgList?.length) {
            pkgList = pkgList.concat(loopPkgMap.pkgList);
            frozen = loopPkgMap.frozen;
          }
          if (loopPkgMap.formulationList?.length) {
            formulationList = formulationList.concat(loopPkgMap.formulationList);
          }
          if (loopPkgMap.dependenciesList) {
            dependencies = mergeDependencies(
              dependencies,
              loopPkgMap.dependenciesList,
              parentComponent,
            );
          }
          // Add the root packages from this file to the parent's dependencies
          for (const p of loopPkgMap.rootList) {
            if (
              parentComponent &&
        ...
        pkgMap = await getPipFrozenTree(path, undefined, tempDir, parentComponent);
      }
