    ...
    for (const f of pomFiles) {
      ...
      if (
        ...
        if (result.status !== 0 || result.error) {
          ...
          thoughtLog(
            "There appear to be build errors, so the SBOM will be incomplete.",
          );
...
export async function createMultiXBom(pathList, options) {
  ...
  thoughtLog("Alright, let's create an xBOM for all languages.");
  ...
  for (const path of pathList) {
    ...
    if (hasAnyProjectType(["oci", "js"], options)) {
      thoughtLog(
        "Now looking for JavaScript projects (npm, yarn, pnpm) and files.",
      );
    ...
    if (hasAnyProjectType(["oci", "java"], options)) {
      thoughtLog(
        "Looking for Java projects (e.g., Maven, Gradle, SBT). I hope all configurations—from Java version to individual build settings—are correctly aligned.",
      );
    ...
    if (hasAnyProjectType(["oci", "py"], options)) {
      thoughtLog(
        "Looking for Python projects with package managers such as pip, poetry, uv, etc. Wish me good luck!",
      );
      if (process.env?.CDXGEN_IN_CONTAINER !== "true") {
        thoughtLog(
    ...
    if (hasAnyProjectType(["oci", "go"], options)) {
      thoughtLog(
        "Looking for go projects. I need to be cautious about purl namespaces and potential failures with the 'go list' command.",
      );
    ...
    if (hasAnyProjectType(["oci", "rust"], options)) {
      thoughtLog(
        "Let's search for Cargo/Rust projects. Should I warn the user that we don't support Cargo 'features' and native dependencies, which may lead to both false positives and false negatives? 🤔?",
      );
    ...
    if (hasAnyProjectType(["oci", "php"], options)) {
      thoughtLog(
        "About to search for Composer-based projects. I hope lock files are available; otherwise, the 'composer install' command might fail for various reasons.",
      );
    ...
    if (hasAnyProjectType(["oci", "ruby"], options)) {
      thoughtLog(
        "Are there any Ruby projects in this path? There's only one way to know.",
      );
    ...
    if (hasAnyProjectType(["oci", "csharp"], options)) {
      thoughtLog("What about csharp and fsharp projects?");
    ...
    if (hasAnyProjectType(["oci", "dart"], options)) {
      thoughtLog(
        "Looking for Dart projects. These are rare ones. Should I inform the user that they can pass the types argument via the command-line to speed things up?",
      );
    ...
    if (hasAnyProjectType(["oci", "haskell"], options)) {
      thoughtLog("Looking for Haskell projects. They're rarely encountered.");
    ...
    if (hasAnyProjectType(["oci", "elixir"], options)) {
      thoughtLog("Looking for Elixir projects. They're quite rare as well.");
    ...
    if (hasAnyProjectType(["oci", "clojure"], options)) {
      thoughtLog(
        "Looking for Clojure projects. Should I warn the user that the purl namespace 'clojars' isn't widely supported by tools like Dependency-Track?",
      );
    ...
    if (hasAnyProjectType(["oci", "github"], options)) {
      thoughtLog("Looking for any github packages and workflows.");
    ...
    if (hasAnyProjectType(["oci", "cloudbuild"], options)) {
      thoughtLog(
        "Let's check for CloudBuild configuration files that include package dependencies.",
      );
    ...
    if (hasAnyProjectType(["oci", "swift"], options)) {
      thoughtLog(
        "Now checking for Swift projects. We don't support CocoaPods, Objective-C, or pure Xcode projects, so the SBOM will be incomplete.",
      );
    ...
    if (hasAnyProjectType(["oci", "jar", "war", "ear"], options)) {
      thoughtLog(
        "Let's check for any bundled jar/war/ear files to improve the SBOM accuracy.",
      );
    ...
    if (options.specVersion >= 1.6 && options.includeCrypto) {
      thoughtLog(
        "Wait, the user wants me to look for cryptographic assets. Let's check thoroughly.",
      );
  ...
  // Retain the components of parent component
  if (parentSubComponents.length) {
    thoughtLog("Tweaking the parent component hierarchy.");
    if (!parentComponent || !Object.keys(parentComponent).length) {
