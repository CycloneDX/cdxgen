        ...
        for (const f of reqFiles) {
          ...
          if (directDependencies?.length) {
            ...
            directDependencies.forEach(pkg => {
              ...
              if (!pkg.evidence) {
                pkg.evidence = { identity: { field: "purl", confidence: 1, methods: [] } };
              }
          ...
          if (options.installDeps) {
            ...
            if (pkgMap.pkgList?.length) {
              pkgMap.pkgList.forEach(pkg => {
                const existingTechnique = packageTechniqueMap.get(pkg.name.toLowerCase());
                if (existingTechnique) {
                  // Update evidence to preserve original technique
                  if (pkg.evidence?.identity?.methods) {
                    pkg.evidence.identity.methods = pkg.evidence.identity.methods.map(method => ({
                      ...method,
