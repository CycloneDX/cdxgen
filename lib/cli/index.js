...
switch (_platform()) {
  case "win32":
    osQueries = JSON.parse(
      await import("../../data/queries-win.json", {assert: {type: "json"}})
    );
    break;
  case "darwin":
    osQueries = JSON.parse(
      await import("../../data/queries-darwin.json", {assert: {type: "json"}})
    );
    break;
  default:
    osQueries = JSON.parse(
      await import("../../data/queries.json"), {assert: {type: "json"}})
    );
    break;
}
const cosDbQueries = JSON.parse(
  await import("../../data/cosdb-queries.json"), {assert: {type: "json"}})
);
