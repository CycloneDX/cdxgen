...
const { default: cosDbQueries } = await import(
  "../../data/cosdb-queries.json",
  importConfig
);
const { default: darwinOsQueries } = await import(
  "../../data/queries-darwin.json",
  importConfig
);
const { default: winOsQueries } = await import(
  "../../data/queries-win.json",
  importConfig
);
const { default: defaultOsQueries } = await import(
  "../../data/queries.json",
  importConfig
);
...
switch (_platform()) {
  case "win32":
    osQueries = winOsQueries;
    break;
  case "darwin":
    osQueries = darwinOsQueries;
    break;
  default:
    osQueries = defaultOsQueries;
    break;
}
    ...
    if (sbtLockFiles?.length) {
      ...
      for (const i in sbtProjects) {
        ...
        if (standalonePluginFile) {
          ...
          if (useSlashSyntax) {
            sbtArgs = [
              `'set ThisBuild / asciiGraphWidth := 800' compile "dependencyTree / toFile ${dlFile} --force"`,
            ];
          } else {
            sbtArgs = [
              `'set asciiGraphWidth in ThisBuild := 800' compile "dependencyTree::toFile ${dlFile} --force"`,
            ];
          }
