      ...
      if (layerConfig) {
        ...
        if (env && Array.isArray(env) && env.length) {
          ...
          if (
            ...
            mproperties.push({
              name: "oci:image:bundlesAndroidSdk",
              value: "true",
            });
          }
          ...
          if (evalue.includes(".sdkman")) {
            mproperties.push({
              name: "oci:image:bundlesSdkman",
              value: "true",
            });
          }
          if (evalue.includes(".nvm")) {
            mproperties.push({
              name: "oci:image:bundlesNvm",
              value: "true",
            });
          }
          if (evalue.includes(".rbenv")) {
            mproperties.push({
              name: "oci:image:bundlesRbenv",
              value: "true",
            });
          }
        }
...
export async function createMultiXBom(pathList, options) {
  ...
  if (
    ...
    if (DEBUG_MODE) {
      console.log(
        `**OS**: Found ${osPackages.length} OS packages and ${executables.length} executables at ${options.allLayersExplodedDir}`,
      );
    }
    if (osPackages.length) {
      ...
      thoughtLog(
        `I couldn't find any OS packages, but I found ${executables.length} executables at ${options.allLayersExplodedDir}. Perhaps the binary plugin wasn't available, or the architecture is unsupported.`,
      );
    } else {
      thoughtLog(
