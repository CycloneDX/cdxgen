...
import calcDepFlags from "../calc-dep-flags.js";
    ...
    async #loadFSNode({
      ...
      try {
        ...
        if (useRootOverrides && root.overrides) {
          params.overrides = root.overrides.getNodeRule({
            name: pkg.name,
            version: pkg.version,
          });
        }
          ...
          for (const p of walkUp(dirname(node.realpath))) {
            ...
            if (!nmContents.has(p)) {
              entries = await readdirScoped(`${p}/node_modules`)
                .catch(() => [])
                .then((paths) => paths.map((p) => p.replace(/\\/g, "/")));
