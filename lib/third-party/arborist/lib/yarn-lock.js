...
const priorityThenLocaleCompare = (a, b) => {
  ...
  return (yarnEntryPriorities[a] || 100) > (yarnEntryPriorities[b] || 100)
    ? 1
    : -1;
};
...
class YarnLockEntry {
  ...
  toString() {
    ...
    return (
      [...this.#specs].sort(localeCompare).map(quoteIfNeeded).join(", ") +
      ":\n" +
...
class YarnLock {
  ...
  parse(data) {
    ...
    while ((match = linere.exec(data))) {
      ...
      if (ENTRY_START.test(line)) {
        ...
        specs.forEach((spec) => {
          this.entries.set(spec, this.current);
        });
        continue;
      }
      if (SUBKEY.test(line)) {
  ...
  toString() {
    return (
      prefix +
      [...new Set([...this.entries.values()])]
        .map((e) => e.toString())
        .sort((a, b) => localeCompare(a.replace(/"/g, ""), b.replace(/"/g, "")))
        .join("\n\n") +
      "\n"
    );
  }
