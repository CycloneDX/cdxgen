...
import gatherDepSet from "./gather-dep-set.js";
import Inventory from "./inventory.js";
import OverrideSet from "./override-set.js";
import printableTree from "./printable.js";
import relpath from "./relpath.js";
import treeCheck from "./tree-check.js";
...
class Node {
  ...
  constructor(options) {
    ...
    const pkg = sourceReference
      ? sourceReference.package
      : rpj.normalize(options.pkg || {});
    this.name =
      name || nameFromFolder(path || pkg.name || realpath) || pkg.name || null;
  ...
  get pkgid() {
    ...
    const alias =
      !isProjectRoot && name && myname !== name ? `npm:${name}@` : "";
  ...
  *ancestry() {
  ...
  get inShrinkwrap() {
    return (
      this.parent && (this.parent.hasShrinkwrap || this.parent.inShrinkwrap)
    );
  }
  ...
  updateOverridesEdgeInRemoved(otherOverrideSet) {
    ...
    for (const edge of this.edgesIn) {
      if (newOverrideSet && edge.overrides) {
        newOverrideSet = OverrideSet.findSpecificOverrideSet(
          edge.overrides,
          newOverrideSet,
        );
      } else {
  ...
  updateOverridesEdgeInAdded(otherOverrideSet) {
    ...
    const newOverrideSet = OverrideSet.findSpecificOverrideSet(
      this.overrides,
      otherOverrideSet,
    );
    if (newOverrideSet) {
  ...
  [_reloadNamedEdges](name, rootLoc = this.location) {
    ...
    const rootLocResolved =
      edge?.to && edge.to.location === `${rootLoc}/node_modules/${edge.name}`;
