...
const depValid = (child, requested, requestor) => {
  ...
  if (typeof requested === "string") {
    try {
      ...
      // '' is equivalent to '*'
      requested = npa.resolve(
        child.name,
        requested || "*",
        fromPath(requestor, requestor.edgesOut.get(child.name)),
      );
    } catch (er) {
  ...
  switch (requested.type) {
    // biome-ignore-start lint/suspicious/noFallthroughSwitchClause: needed
    case "range":
      if (requested.fetchSpec === "*") {
        return true;
      }
    // biome-ignore-end lint/suspicious/noFallthroughSwitchClause: needed
    // fallthrough
    case "version":
...
const tarballValid = (child, requested) => {
  ...
  if (child.resolved) {
    return (
      child.resolved.replace(/\\/g, "/") ===
      `file:${requested.fetchSpec.replace(/\\/g, "/")}`
    );
  }
