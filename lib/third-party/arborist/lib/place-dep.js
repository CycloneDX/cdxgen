...
import deepestNestingTarget from "./deepest-nesting-target.js";
import gatherDepSet from "./gather-dep-set.js";
import Link from "./link.js";
import peerEntrySets from "./peer-entry-sets.js";
...
class PlaceDep {
  constructor(options) {
    ...
    this.placed = new this.dep.constructor({
      ...
      ...(this.dep.isLink
        ? { target: this.dep.target, realpath: this.dep.realpath }
        : {}),
    });
  ...
  explainPeerConflict(edge, dep) {
    ...
    if (this.parent) {
      ...
      expl.current = curNode?.explain(edge);
      expl.peerConflict = this.current?.explain(this.edge);
    } else {
      expl.current = curNode?.explain();
