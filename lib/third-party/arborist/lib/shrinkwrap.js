...
import normalize from "@npmcli/package-json";
import stringify from "json-stringify-nice";
import npa, { resolve as _resolve } from "npm-package-arg";
import parseJSON from "parse-conflict-json";
import log from "proc-log";

import specFromLock from "./spec-from-lock.js";
import versionFromTgz from "./version-from-tgz.js";
...
import consistentResolve from "./consistent-resolve.js";
import { overrideResolves } from "./override-resolves.js";
...
class Shrinkwrap {
  ...
  static metaFromNode(node, path, options = {}) {
    ...
    if (
      ...
      (node === node.root ||
        pname !== node.name ||
        nameFromFolder(node.realpath) !== pname)
    ) {
      meta.name = pname;
    }
  ...
  constructor(options = {}) {
    ...
    if (hiddenLockfile) {
      ...
      this.lockfileVersion = Number.parseInt(lockfileVersion, 10);
    } else {
  ...
  checkYarnLock(spec, options = {}) {
    ...
    if (fromYarn?.version) {
      ...
      if (yspec) {
        options.resolved = resolved.replace(
          yarnRegRe,
  ...
  async load() {
    ...
    this.ancientLockfile =
      this.loadedFromDisk && !(data.lockfileVersion >= 2) && !data.requires;

    // load old lockfile deps into the packages listing
    if (data.dependencies && !data.packages) {
      let pkg;
      try {
        pkg = await normalize(this.path);
        pkg = pkg.content;
      } catch {
        pkg = {};
      }
  ...
  #resolveMetaNode(loc, name) {
    // biome-ignore lint/correctness/noConstantCondition: needed
  ...
  #metaFromLock(location, name, lock) {
    ...
    if (!meta.version) {
      if (spec.type === "file" || spec.type === "remote") {
        const fromTgz =
          versionFromTgz(spec.name, spec.fetchSpec) ||
  ...
  add(node) {
    ...
    if (node.resolved === null || node.integrity === null) {
      const { resolved, integrity, hasShrinkwrap, version } = this.get(
        node.path,
      );
      ...
      const resolvedOk =
        !resolved || !node.resolved || node.resolved === pathFixed;
      const integrityOk =
        !integrity || !node.integrity || node.integrity === integrity;
      ...
      if (allOk) {
        ...
        const { resolved, integrity, hasShrinkwrap } = Shrinkwrap.metaFromNode(
          node,
          this.path,
          this.resolveOptions,
        );
  ...
  #buildLegacyLockfile(node, lock, path = []) {
    ...
    let spec = rSpec;
    if (edge) {
      spec = _resolve(node.name, edge.spec, edge.from.realpath);
    }
