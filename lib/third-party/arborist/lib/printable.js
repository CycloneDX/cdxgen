...
class ArboristNode {
  constructor(tree, path) {
    ...
    const bd = tree.package?.bundleDependencies;
    if (bd?.length) {
    ...
    if (tree.overrides) {
      this.overrides = new Map(
        [...tree.overrides.ruleset.values()].map((override) => [
          override.key,
          override.value,
        ]),
      );
    ...
    if (tree.workspaces?.size) {
      this.workspaces = new Map(
        [...tree.workspaces.entries()].map(([name, path]) => [
          name,
          relpath(tree.root.realpath, path),
        ]),
      );
    }
