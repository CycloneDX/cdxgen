...
export function getBinaryBom(src, binaryBomFile, deepMode) {
  ...
  // Validate src parameter is under tmpdir to address CodeQL path expression warning
  if (typeof src !== 'string' || src.length === 0) {
    console.log(`Invalid source path: ${src}`);
    return false;
  }
  const tmpRoot = getTmpDir();
  const resolvedSrc = resolve(src);
  const rel = relative(tmpRoot, resolvedSrc);
  if (rel.startsWith('..') || resolve(tmpRoot, rel) !== resolvedSrc) {
    console.log(`Source path ${src} is not under tmp directory ${tmpRoot}`);
    return false;
  }
