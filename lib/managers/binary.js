...
export function getBinaryBom(src, binaryBomFile, deepMode) {
  ...
  // Resolve and validate the input path early to avoid using uncontrolled data in path expressions
  const absSrc = resolve(src);
  const args = ["sbom", "-i", absSrc, "-o", binaryBomFile];
  if (deepMode) {
    args.push("--deep");
  }
  if (DEBUG_MODE) {
  ...
  const lstatResult = lstatSync(absSrc, { throwIfNoEntry: false });
  if (!lstatResult) {
    console.log(`Source path ${absSrc} does not exist`);
    return false;
  }
