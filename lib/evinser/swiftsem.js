  ...
  if (dumpJson.targets) {
    for (const atarget of dumpJson.targets) {
      ...
      if (atarget.dependencies) {
        const dependsOn = atarget.dependencies.map((v) =>
...
export function createSemanticsSlices(basePath, options) {
  ...
  if (!compilerArgs || !sdkArgs) {
    ...
    if (!sdkArgs) {
      console.log(
        "Unable to detect the swift sdk needed to build this project. Try running the swift build command to check if this project builds successfully.",
      );
      return;
    }
  }
