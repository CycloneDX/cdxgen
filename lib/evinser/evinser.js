...
export async function createSlice(
  ...
  const result = executeAtom(filePath, args, {
    ATOM_TOOLS_OPENAPI_FILENAME: openapiSpecFile,
    ATOM_TOOLS_OPENAPI_FORMAT:
      process.env?.ATOM_TOOLS_OPENAPI_FORMAT || "openapi3.1.0",
    ATOM_TOOLS_WORK_DIR:
      process.env?.ATOM_TOOLS_WORK_DIR || join(getTmpDir(), "atom-tools-"),
  ...
  } else if (sliceType === "usages" && !safeExistsSync(openapiSpecFile)) {
    console.log(
      `openapi spec file "${openapiSpecFile}" was not generated successfully. Check if atom-tools pypi package is installed and available in PATH.`,
    );
  }
  return {
