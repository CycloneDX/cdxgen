...
export async function createSlice(
  ...
  const result = executeAtom(filePath, args, {
    ATOM_TOOLS_OPENAPI_FILENAME: openapiFile,
  ...
  } else if (sliceType === "usages" && !safeExistsSync(openapiFile)) {
    console.log(
      `openapi spec file "${openapiFile}" was not generated successfully. Check if atom-tools pypi package is installed and available in PATH.`,
    );
  }
  return {
